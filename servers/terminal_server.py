...
mcp = FastMCP("terminal_server") #defines mcp server

#defining workspace where this terminal server will work and acces the files
DEFAULT_WORKSPACE = os.path.expanduser("~\\prathamesh\\ai_agent\\mcp_a2a_multiagent")
...
async def run_command(command:str) -> str:
    ...
    """
    try:
        result = subprocess.run(command, shell=True, cwd = DEFAULT_WORKSPACE, check=True, text= True, capture_output= True)
        return result.stdout or result.stderr
    except Exception as e:
